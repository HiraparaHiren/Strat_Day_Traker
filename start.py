...
def fetch_data(symbol, data_to_string, dataframes, lock):
    obj = MControlAPI(symbol, data_to=data_to_string)
    df = obj.fetch_intraday_data()
    if not df.empty:
        with lock:
...
def main():
    ...
    if entered_password == "///":
        ...
        if st.button("Get Data"):
            ...
            if not st.session_state.data_fetching:
                ...
                for symbol in symbols:
                    thread = threading.Thread(target=fetch_data, args=(symbol, data_to_string, dataframes, lock))
